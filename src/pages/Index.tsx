...
import { chats as initialChats, messages as initialMessages, Message, BotFatherState, CreatedBot, handleBotFatherMessage } from "@/data/mockData";
...
const Index = () => {
  const [activeChatId, setActiveChatId] = useState<string | null>("chat-botfather");
  ...
  const [createdBots, setCreatedBots] = useState<CreatedBot[]>([
    { name: 'MockBot', username: 'mock_test_bot', token: '8399914870:AAH3mANGZFUfqAU8kf1HvOHCNNvr-j6RagY', createdAt: new Date().toISOString() },
  ]);
  ...
  const handleSend = useCallback((text: string) => {
    ...
    if (activeChatId === 'chat-botfather') {
      setTimeout(() => {
        const result = handleBotFatherMessage(text, bfState, createdBots, bfPendingName);
        ...
        if (result.deletedUsername) {
          setCreatedBots(prev => prev.filter(b => b.username !== result.deletedUsername));
        }
        if (result.renamedBot) {
          setCreatedBots(prev => prev.map(b => 
