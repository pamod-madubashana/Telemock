...
function renderTextWithCommands(text: string, onCommandClick?: (cmd: string) => void) {
    ...
    if (/^\/[a-zA-Z_][a-zA-Z0-9_]*(?:@\w+)?$/.test(part)) {
      return (
        <span
          ...
          onClick={(e) => { e.stopPropagation(); onCommandClick?.(part); }}
        >
          {part}
        </span>
      );
...
export function MessageBubble({ message, isOwnMessage, showSender, onCommandClick }: MessageBubbleProps) {
  ...
  return (
    <div className={cn("flex mb-1.5", isOwnMessage ? "justify-end" : "justify-start")}>
      <div
        className={cn(
      ...
      >
        {showSender && !isOwnMessage && sender && (
          <p className="text-xs font-semibold mb-0.5" style={{ color: sender.color }}>
            {sender.name}
            {sender.isBot && <span className="ml-1 text-[10px] font-normal text-muted-foreground">bot</span>}
          </p>
        )}
        {message.replyTo && (
        ...
        <div className={cn("flex items-center justify-end gap-1 mt-0.5", isOwnMessage ? "-mb-0.5" : "")}>
          <span className="text-[11px] text-timestamp">{message.timestamp}</span>
          {isOwnMessage && (
            message.read
